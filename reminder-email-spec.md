...
Read at run time by the two scheduled tasks, **GF timetable — morning reminder (06:30)** and **GF timetable — evening recap (20:00)**. Edit this file, push it, and the next run picks the changes up — no need to touch either task.
...
- The morning task fires at **10:30 UTC** = 06:30 the **same** local date. Local date = UTC date.
- The evening task fires at **00:00 UTC** = 20:00 the **previous** local day. The day it recaps is **UTC date − 1**; "tomorrow" in that email is the UTC date.

Compute this from `date -u` in a script. This is the single most likely place for the emails to go a day out.
...
## Evening email — 20:00, recaps today and previews tomorrow
